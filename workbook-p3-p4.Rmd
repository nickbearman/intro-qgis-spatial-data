...
output:
  ...
  html_document:
    df_print: paged
date: 'Practicals 3 & 4: Tue 22nd and Wed 23rd March 2022'
---
...
- Open **QGIS** and use the **Open Data Source Manager** ![](images/open-data-source-manager.png){ height=20 } 
...
- If you have time, try loading in the Style template that we used before  `imd-d-style.qml`.  
...
- Save this layer (`MYE2 - Persons`) into either the geopackage you saved in the previous practical, or save it to a new geopackage ((**right-click > Export > Save Features As...**))  

<!-- Be clearer this is just a table we are saving - it won't have a CRS --> 

- Return to the **Fields** window and now we should be able to edit ![](images/edit-icon.png){ height=20 } .  
...
- Click **Run**.  
- It may give a warning message - we need to check the output.  
...
- Use the **Fields** view in **Properties** to check that the **Refactored** layer now has the population data in the correct data type.  
...
- Click the link **OS Open Greenspace - ESRI® Shapefile SU (2.27 MB)** to download.  
...
Now we are going to group the data within each Local Authority. First, we will use **Point-in-Polygons** to count how access points to greenspace there are in each Local Authority. 

- Click **Processing > Toolbox**.  
- Type `point in polygons` in the search box.  
- Double-click **Vector analysis tools > Count points in polygons**.  
- Set **Polygons** to your Local Authority districts layer.   
- *(or `Local_Authority_Districts_April_2019_Boundaries_Southern`).*  
...
- It should give you a preview value for **Arun** of `1.21`.  
- Click **OK**.  
- Save the data.  
...
- Open up your project file from the previous practical, and save it as a new file.  
...
- To do this, right click on the layer, choose **Export > Save Features As**.  
...
- We also need to do one final bit of preparation on the local authority data - add the area.  
...
- This should come out (as a preview)  at `634273162` (m) for Basingstoke and Deane.    
...
- In the greenspace layer add a new column called `greenspace`.  
...
*We do have quite a few completely blank rows (with greenspace data but no LSOA data). These are the greenspace areas that are outside our local authority data. Try selecting one and using the Zoom To function.*

- Save this as a new layer called `greenspace-LAs`. It won't save in the geopackage (because `fid` is not unique, one of the requirements), so save it as an ESRI Shapefile.  
...
- Expand **Geometry** and double-click `$area`. This will add the area in square meters (as our coordinate system is BNG).  
- This should come out (as a preview)  at `93762.20` (m).  
- Add `/ 10000` to our expression to divide this by 10,000, to get 9.37 hectares.  
- Click **OK** to apply the calculation across all the rows.  
- Turn Editing off.  
...
- Save as `dissolved-greenspace-LAs.shp`.  
...
- Save this as CSV file (*right-click > Export > Save Selected Features As*) called `greenspace-LAs-pc.csv`. Remember to tick the **Save only selected features** checkbox.  
- In the CSV file, use **Field Calculator** to create a new field (`pc_greenspace`) and calculate the percentage of greenspace (`("area"  /  "LAarea" ) * 100`).  
...
*This practical was written using QGIS 3.16.0 on Windows 10, and tested in QGIS 3.10.11, last updated on 23/02/2022 by Dr Nick Bearman, [nick@geospatialtrainingsolutions.co.uk](nick@geospatialtrainingsolutions.co.uk).*
