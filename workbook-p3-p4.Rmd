...
- Open this file in Excel to have a look at it. 
  - Note we have columns called `CTYUA24CD` - this is the code for the Local Authority which we will be using to join the data and `CTYUA24NM` which is the name of the Local Authority. We also have `RAvgRank`: (Rank of) Average Rank, 	`RAvgScor`: (Rank of) Average Score and `RPLMD10`: Proportion of LSOAs in most deprived 10% nationally. We are going to use (Rank of) Average Rank as the summary for our analysis. See section [7 Local Authority District Analysis](https://www.gov.uk/government/statistics/english-indices-of-deprivation-2025/english-indices-of-deprivation-2025-statistical-release#local-authority-district-analysis) for more details. 
...
The join that we have made is not permanent; it is a temporary link between the two datasets. To make the Join permanent we need to save a new copy of the local authorities layer. Try saving this as a new GeoPackage. 
...
*We had to use **Refactor** because QGIS can't read Excel files in the same way it can read CSV files. An alternative method is to save the MYE 2 - All Persons tab from the Excel file as a CSV file, and then read that in using Add Delimited Text, like we did before. Then we wouldn't need to worry about Refactor. I just wanted to show you the options, but usually I would recommend saving Excel files as a CSV file to make things easier!* 
...
- In the `data` folder there should be two shape files `SU_AccessPoint.shp` and `SU_GreenspaceSite.shp`, `TQ_AccessPoint.shp` and `TQ_GreenspaceSite.shp`.  
...
We have a couple of data management issues that you may have spotted. Firstly, the greenspace access points data is split over two files - `SU_AccessPoint.shp` and `TQ_AccessPoint.shp`. Secondly, we do not have complete greenspace data for all of the areas in the `southern_englandcounties_unitary_authorities_dec_2024_imd` layer. We can solve both of these problem and will do in the next section.
...
![](images/save-layer-geopackage-greenspace.png)
...
There is one outlier - an area with >6000 NUMPOINTS (Greenspace access points). We can customise the plot and ignore the value of >6000. Click on the Cogs icon (top left) to return to the **Layout Options** page. Check **Set X Axis Bounds** and type **2000** in **Maximum**. Cluck Update Plot and have another look. 
...
- It is always good practice to work on a copy of your data, so take a copy of our geopackage from the previous practical (by copying it in Windows Explorer / My Computer / Finder) and save it in the `prac4` folder. I have called mine `prac3-data.gpkg`.  
- Then add your new GeoPackage to QGIS.  
- Then we can add the greenspace area data to this prac3-data.gpkg geopackage.  
- To do this, right click on the layer (`SU_GreenspaceSite`), choose **Export > Save Features As**.  
- Select the `prac3-data.gpkg` geopackage.  
- Call the layer `SU_GreenspaceSite` and choose **OK**.  
- It will appear as `prac3-data — SU_GreenspaceSite`.  
...
- Save the local authorities layer (which should already have the IMD and population data in it) into our `prac3-data` geopackage using the same method as above. Call it `lad-imd-pop-SUgreenspace`.  
- Remove the `prac3-data lad-imd-pop-SUgreenspace` layer, so we only have one local authority layer.  
- Select the `prac3-data lad-imd-pop-SUgreenspace` layer and start editing ![](images/edit-icon.png){ height=20 }. 
...
- Open the attribute table for the `prac3-lad-imd-pop-SUgreenspace` layer.  
...
- In the `prac3-data — GreenspaceSite` layer add a new column called `greenspace`.  
...
- Set the **Input layer** to `prac3-data lad-imd-pop-SUgreenspace` (our local authority data).  
- Set the **Overlay layer** to `prac3-data SU_GreenspaceSite` (the greenspace site layer).  
...
<!-- This is only a problem in QGIS 3.38 or below. It is fine and doesn't give an error in QGIS 3.40.12 or 3.44.4 -->
...
*This practical was written using QGIS 3.40 on Windows 11, last updated on 23/10/2025 by Dr Nick Bearman, [nick@nickbearman.com](mailto:nick@nickbearman.com).*
