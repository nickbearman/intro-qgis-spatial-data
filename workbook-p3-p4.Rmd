...
- Click **Download** and then choose **CSV**. Save this file somewhere you can find it (e.g. in a new folder called `prac3`). 

- Open this file in Excel to have a look at it. We have a number of columns: 
  - `CTYUA24CD` this is the code for the Local Authority which we will be using to join the data 
  - `CTYUA24NM` which is the name of the Local Authority
...
- In QGIS open the **Open Data Source Manager** ![](images/open-data-source-manager.png){ height=20 } and use the **Delimited Text Layer** option to add the CSV file you have just downloaded.
...
- In the **Sample Data** option, check the data types - our code and name files should be set to **Text** and our rank and score fields set to **Integer**. 
...
- Double click the `RAveRank` and `RAvgScor` fields with the long pre-fix and rename them to just `RAveRank` and `RAvgScor`.
...
- Select the `RAveRank` field from the column drop-down list.  
...
- Remove the old `MYE2 - Persons` layer so we don't get confused.
...
- Zoom in and have a look at the points. It may be useful to add the OSM Basemap layer, to provide some context *(Hint: XYZ Tiles in the Broswer panel)*.  
...
- Save this new temporary layer as `AccessPoint` within a new Geopackage file called `prac3-data.gpkg`

To select the polygons we have complete point data for, we have to do this manually. 

- To start with, make sure you only have the two layers visible - `prac4-data — AccessPoint` which are orange points below, and `prac3-data — counties_unitary_authorities_dec_2024_imd_pop2024` which are purple polygons. *Note, your colours may vary*. 
