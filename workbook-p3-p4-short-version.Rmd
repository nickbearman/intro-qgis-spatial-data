...
output:
  ...
  html_document:
    df_print: paged
date: 'Practicals 3 & 4: Tue 23rd July 2024'
---
...
*See here https://youtu.be/HbZiZOsHD8A for a video if you want to follow along.*
...
- Expand the **Geopackage** item and then the `prac3-data.gpkg` file and your **Browser** window should look like this:

![](images/browser-window.png){ width=40% }

- Add the 
`LA-IMD-pop` layer by double clicking on it.  
...
- Pan to the Winchester local authority (try labelling them as we did with the World Cities yesterday if you need to).  
...
- If the calculation has worked, save this as a new GeoPackage layer, by right-clicking on the layer and choosing **Export > Save Features As...**.  
- Remember to choose the GeoPackage file name and a suitable name for the layer name:
...
*If `IMD_IMD - Average score` isn't visible, you may have not done a join correctly earlier on. Try a different field, and try re-doing the join.*
...
# Practical 4: Spatial Overlays - Greenspace and Local Authorities
...
*This practical was written using QGIS 3.34 on Windows 11, last updated on 29/02/2024 by Dr Nick Bearman, [nick@nickbearman.com](mailto:nick@nickbearman.com)).*
