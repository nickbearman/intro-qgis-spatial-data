...
date: "Practicals 1 & 2: Tue 4th & Wed 5th August 2026"
---
...
- Open the InStats site <!-- OneDrive folder --> and find the `prac1.zip` file.  
...
- Start QGIS by clicking on Start, then type **QGIS** and select **QGIS Desktop 3.44.11**. *Your version number might vary, **QGIS Desktop** is the key bit.*  
...
### Getting and symbolizing the data
...
- Select the `southern_england-lsoa2021_imd2025` layer and choose **Add**. Then choose **Close**.  
...
- *You may get a **Select Transformation for OpenStreetMap** window. This is just saying that QGIS is reprojecting the data from 3857 (what OpenStreetMap comes in) to 27700 (British National Grid), which is what we are using. Click **OK**.*
- You may need to re-order the layers if the basemap is added at the top.  

----

*If this doesn't work, try installing the **QuickMapServices** plugin:*				   
...
- When you have finished looking at the map, right-click on the `southern_england-lsoa2021_imd2025` layer and choose **Zoom to Layer**  ![](images/zoom-to-layer-right-click.png){ height=20 } to return to our IMD data.  
- Right click on the `southern_england-lsoa2021_imd2025` layer and choose **Properties > Symbology**.  
- Change the **Opacity** to **50%**. 
- If you can't see it, expand **Layer Rendering** at the bottom of the window. Alter the **Opacity** option to **50%**.
- Choose **OK**.  
...
The default colours for the IMD data aren't that great, but fortunately we have a Style file, created by Alasdair Rae for the 2019 IMD data (see https://www.sheffield.ac.uk/usp/research/projects/english-indices-deprivation-2019 for more details). 

- Right click on the `southern_england-lsoa2021_imd2025` layer, choose **Properties > Symbology**.  
...
- Click **Load Style**.
- As this style sheet is from the 2019 IMD data, it is looking for a column called `IMD_Decile` but in our data the column we need to use is called `IMDDecil`. Change the **Value** field to `IMDDecil` and click **OK**. This will load the colours for our IMD Decile layer.  

We can also add some outlines for the Local Authority Districts.   

- Add in the `southern_england-counties_unitary_authorities_dec_2024` layer from the same geopackage as we used earlier.  
...
- Set **Value** to `CTYUA24NM` and choose **OK**. *This is the column from the attribute table of the Local Authority layer that we are using for the labels. If you want, have a look at the attribute table and see what is in each column.*  
- *If nothing appears, make sure you have selected `CTYUA24NM`*.  

This adds the Local Authority boundaries to give us some context for the IMD data.

**Optional Exercise**

Try experimenting with adjusting the transparency - which do you think looks better?
...
- Centre the map display in the main QGIS window. Make sure you can see the area in the map clearly.  
...
- Draw a box about the appropriate size of the main map on the page. It should look something like this:  
...
- We need to make the labels a bit clearer. Go back from the Layout window to the main QGIS window, and open the **Symbology** window for the `southern_england-counties_unitary_authorities_dec_2024` layer.  
...
- You might also need to adjust the scale to get the map looking right. I had a value of around `640000` in the scale box. To adjust, type a number in and press Enter.  
- You can also move the map around with the map window using the **Move item content** tool ![](images/move-item-content.png){ height=20}. If you have a mouse with a mouse wheel, you can also zoom in and out on the data using the wheel. Use **Ctrl** with the mouse wheel to zoom in or out in smaller increments.    
...
You may have noticed that the area around London, in the top-right of the map, is a bit congested. This is very common when mapping using Census geographies in densely populated urban areas. One way around this is to have an inset map, showing that area in more detail, which is what we are going to add now for London. 

- Go back to the main **QGIS** window and zoom in on London.  
- Then go back to the **Layout** window and add a new map on the top-right of the page using ![](images/add-new-map.png){ height=20}.  
- It should look something like this:

![](images/layout-2.png){ width=60% }

*What do you think of this so far?*

- This is ok, but it would be better to make London stand out a bit more. We are going to add the Region boundaries, which will outline London. 

- Go back to the main QGIS window.  
- Add the `regions_december_2024_boundaries` layer from our `prac2-data.gpkg` file on top of the other layers. 
- Update the symbology so it is an outline, using the same steps as we did for the unitary authorities layer. 
- This time, set the **Stroke Width** to **0.66**. 
- Go back to the layout window, make sure the London map is selected, and click **Refresh** ![](images/update-map-preview.png){ height=20}.  
- Select the London map, scroll down in the **Item Properties** and check the box next to **Frame**. 
...
![](images/auto-update-off.png){ width=40% }

- Select the `regions_december_2024_boundaries` layer and click the minus ![](images/minus.png){ height=20 } button to remove it.  
- Select the `southern_england-counties_unitary_authorities_dec_2024` layer and choose the **Edit Item** ![](images/rename.png){ height=20 } button. Change the label to `County & Unitary Authorities`.  
- Replace the title of `southern_england-lsoa2021_imd2025` with `Index of Deprivation`.  
- Repeat the same process to update the colours to `1 Most deprived` and `10 Least deprived`.  
- Remove the `OpenStreetMap` entry.  
- Move `County & Unitary Authorities` to the bottom of the list using the **Move item down**  ![](images/move-item-down.png){ height=20}.
- In the **Title** option at the top add in **Index of Deprivation**. 
- Right-click on the `Index of Deprivation` entry under **Legend Items** and choose **Hidden**. 
- Move the legend to fit in the gap on the page.  

*If the legend looks a bit pixilated, click the Refresh button on the layout window (top-left) and it should refresh everything and become nice and sharp.*

- Make sure you have the main map selected (using the **Select/Move item** tool). *It should have 8 small white boxes around the edge when you select it.*
- Click **Add Item > Add Scale Bar** and then click in the gap at the bottom of the page. You may have to click **OK** again.  
- Resize the box so it fits logically on the page. 
- In the Scalebar settings, set **Segments** to `right 9` and leave **Fixed width** as `10 units`. It should automatically default to `km`.  

This scale bar refers to the main map. We also have an inset map, so we need to add a scale bar to that too. Each map will have a number (Map 1 and Map 2) and we can specify which map each scale bar refers to. With the main map, by selecting it, we are saying to QGIS, for this map, add a scale bar. 

- Repeat the same process of adding a scale bar for the London map, making sure you select it first.
- Set **Segments** to `right 5` and set **Fixed width** to `5 units`.
- Also change **Height** from **3mm** to **2mm**. 

Lastly, we need to add the acknowlegdement and coypright information

- The IoD data is made available under the Open Government license (http://www.nationalarchives.gov.uk/doc/open-government-licence/version/3/) so we need to include an acknowledgement on our map. We are also using data from the ONS which is under a different license (https://www.ons.gov.uk/methodology/geography/licences) so we need to add this as well. 
- Click **Add Item > Add Label** and click in the empty space. Type in these details:  
  - `Office for National Statistics licensed under the Open Government Licence v.3.0`
  - `Contains OS data © Crown copyright and database right 2026` 
- *You can copy and paste the copyright symbol from the Wikipedia copyright symbol page, or the workbook!*

You may need to tweak things to get the layout right. It can be a bit of a fiddly process, but do persist and it will look really good when it is done. 
...
### Highlight an area of interest (optional exercise)

![](images/layout-london.png)

If you have time, you can update the London map so it just shows the London Boroughs rather than the whole area. 

Have a look at this video from Map Academy to see the process: https://www.youtube.com/watch?v=SbQGg4GSpPc

You will need to use the **filter layer**, **outline** and **inverted polgyons** options within QGIS to achieve this. 
...
*This practical was written using QGIS 3.44 on Windows 11, last updated on 06/07/2026 by Dr Nick Bearman, [nick@nickbearman.com](mailto:nick@nickbearman.com). Thanks to Chris Bell for providing the geocoding resource at https://www.doogal.co.uk.*
...
The latest version of the workbook is available from https://github.com/nickbearman/intro-qgis-spatial-data. This is v6.3 and this was created on `r format(Sys.time(), '%d %B %Y')`. 




![](images/layout-final-rotated.png){ width=100% }
